...
| Source DEM file content | measured elevations change; generated provenance records `DataSource.sha256` for a single-file GeoTIFF. A detached PDS product records the label and image separately in `DataSource.files[]`, each with its own SHA-256, because both georeferencing metadata and elevation bytes affect the result. Identity is bound to the same opened bytes used for decoding: GeoTIFF parsing and hashing share one in-memory source buffer, while PDS label parsing hashes its opened label bytes and image hashing/window reads share one verified file descriptor. |
| JS engine / libm differences | can perturb the transcendental tier (see above); run the gate, don't assume. The committed oracle (`examples/south_pole_site_01/expected-checksums.sha256`) was regenerated on **Node 26.7.0** (V8 14.6) on 2026-08-17 and CI/`.nvmrc` pin that build. Measured the same day: Node 20.20.2 reproduces 181/183 against it — `craters.json` and `rocks.json` differ in the last ulp of `Math.pow`-derived crater depth/rim/ejecta values; all 181 raster, tile, mesh, horizon and mask artifacts are identical across the two engines |
