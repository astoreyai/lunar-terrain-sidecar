...
- Cumulative **area** fraction covered by rocks ≥ D: `F(D) = k·exp(−q(k)·D)`, `q(k) = 1.79 + 0.152/k`, with `k` the total area fraction (config `cumulativeFractionalAreaCovered`, default 0.06; typical lunar terrain k ≈ 0.03–0.10).
...
- Pre-generation rock **estimates are upper bounds** — taken before slope and rim-density rejection, which depend on terrain not yet generated (`terrain-core/src/estimate.ts`).
