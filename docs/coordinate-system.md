...
- `origin.site` is a `Selenographic` position — latitude positive north, longitude positive east, degrees — in the **Mean Earth / Polar Axis (ME)** body-fixed frame (`CoordinateSystem.body_frame: 'MOON_ME'`). ME is the frame the LOLA gridded products and the PGDA polar site DEMs are published in, so it is the correct frame to pair with them (`lunar-solar/src/lunarFrame.ts` header; realised via the IAU/WGCCRE expressions of Archinal et al. 2011 with the E1…E13 libration terms).
...
- When terrain was ingested from a projected DEM, `CoordinateSystem.source_projection` (`ProjectionMetadata`, type `polar_stereographic`) carries the full projection parameters plus the projected coordinate of the local origin, so a consumer can re-project local metres back to selenographic coordinates exactly.
- The local tangent frame at the site is built by `buildLocalFrame` (`lunar-dem/src/sample.ts`): in the south-polar-stereographic aspect, local east is `(cos λ, −sin λ)` and local south (+Z) is `−(sin λ, cos λ)` in projected space, with a per-latitude projection scale factor.
