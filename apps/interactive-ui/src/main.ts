...
async function connect(): Promise<void> {
  ...
  try {
    ...
    // user regenerates. "No terrain is loaded" is the normal fresh-server
...
/**
 * The lit view has a single solar source and no sky light, so with the Sun
 * below the horizon it renders honest blackness — which reads as "broken"
 * unless labelled. Shown only when terrain is actually loaded: an empty
