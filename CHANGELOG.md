...
- The non-authoritative WebGPU preview accepts hardware adapters only. With the software (SwiftShader) fallback adapter, compute-pipeline compilation monopolised Chromium's GPU process, the three.js WebGL context was lost and the page froze for minutes on a two-core CI runner; without a hardware adapter the preview now reports itself unavailable, which is also the honest answer since a software preview is not faster than the sidecar round trip it exists to hide.
...
- The single-running-job test pipelines both generation requests instead of awaiting the first response, so it no longer depends on client-side scheduling; on a loaded machine the small job could finish before the second request was sent, making a correct refusal look like an acceptance.
